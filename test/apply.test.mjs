...
test('默认段 name = dsh-docs:docs', () => { const c = makeMockCtx(); apply(c, {}); assert.equal(c.sections[0].name, 'dsh-docs:docs') })
test('默认段 order = 61', () => { const c = makeMockCtx(); apply(c, {}); assert.equal(c.sections[0].order, 61) })
test('默认段包含文档标识文本', () => { const c = makeMockCtx(); apply(c, {}); assert.ok((c.sections[0].text || '').includes('技术文档工程师')) })
test('默认段不含 complete', () => { const c = makeMockCtx(); apply(c, {}); assert.equal(c.sections[0].complete, undefined) })
test('config.text 整体覆盖', () => { const c = makeMockCtx(); apply(c, { text: '你是技术写作者' }); assert.equal(c.sections.length, 1); assert.equal(c.sections[0].text, '你是技术写作者'); assert.equal(c.sections[0].name, 'dsh-docs:docs') })
test('自定义 name 生效', () => { const c = makeMockCtx(); apply(c, { name: 'custom:d' }); assert.equal(c.sections[0].name, 'custom:d') })
test('order 数字生效', () => { const c = makeMockCtx(); apply(c, { order: 0 }); assert.equal(c.sections[0].order, 0) })
test('order 非数字回退 61', () => { const c = makeMockCtx(); apply(c, { order: '61' }); assert.equal(c.sections[0].order, 61) })
...
test('[已修复] order:NaN 消毒 61', () => { const c = makeMockCtx(); apply(c, { order: NaN }); assert.equal(c.sections[0].order, 61) })
test('[已修复] order:Infinity 消毒 61', () => { const c = makeMockCtx(); apply(c, { order: Infinity }); assert.equal(c.sections[0].order, 61) })
